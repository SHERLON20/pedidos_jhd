...
def main(page:ft.Page):
    ...
    conn = psycopg2.connect(
            ...
            database="bd_sherlon",  # nome do seu banco de dados
    ...
    def consulta(sql):
        ...
        return rows
    dados_pedidos = consulta("SELECT * FROM pedidos;")
    def deleta_de_vez(e):
        id=int(e.control.parent.parent.data)
        dados_pedidos_delete = consulta("SELECT * FROM pedidos;")
        numero_valor1 = 0
    ...
    def aparece_na_tela_client(e):
        try:
            ...
            if e.control.image.src == 'negativo.webp':
                ...
                e.control.update()
                cursor = conn.cursor()
                cursor.execute("UPDATE pedidos SET visivel = False WHERE id = %s", (id,))
                conn.commit()
                # dado= {
                #     'visivel':False
                # }
                ...
                e.control.update()
                cursor = conn.cursor()
                cursor.execute("UPDATE pedidos SET visivel = True WHERE id = %s", (id,))
                conn.commit()
                # dado= {
                #     'visivel':True
                # }
    ...
    def alterar_delete(e):
        ...
        dados_pedidos_1 = consulta("SELECT * FROM pedidos;")
        numero_valor = 0
        ...
        def sim(e):
            cursor = conn.cursor()
            cursor.execute("UPDATE pedidos SET is_delete = True WHERE id = %s", (id_d,))
            conn.commit()
            page.add(ft.AlertDialog(
                open=True,
    ...
    def select_mesa(e):
        ...
        if e.control.content.value == "TUDO":
            dados_pedidos_2 = consulta("SELECT * FROM pedidos;")
            ...
            dados_pedidos_4 = consulta("SELECT * FROM pedidos;")
            lista_pedido.content.controls = [
                ft.Container(
                    data=item2[0],
                    padding=ft.padding.all(20),
            ...
            total.spans[1].text = 0.0
            total.update()
            dados_pedidos_3 = consulta("SELECT * FROM pedidos;")
                        ...
                        def sim(e):
                            ...
                            for num in mesas.content.controls:
                                if num.bgcolor == ft.Colors.RED_100:
                                    ...
                                    numero = f"{separa[1]}"
                                    cursor = conn.cursor()
                                    cursor.execute("UPDATE pedidos SET is_delete = True WHERE numero_mesa = %s", (numero,))
                                    conn.commit()
                                    page.add(ft.AlertDialog(
                                        open=True,
                                        title='alterado com sucesso!'.upper()
    ...
    while True:
        if topo.content.controls[2].value == True:
            if mesas.content.controls[0].bgcolor == ft.Colors.RED_100:
                data_atualiza = consulta("SELECT * FROM pedidos;")
                lista_pedido.content.controls = [
                    ft.Container(
                        data=item_a[0],
                        padding=ft.padding.all(20),
